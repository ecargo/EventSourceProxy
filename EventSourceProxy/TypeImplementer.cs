...
namespace EventSourceProxy
{
    ...
    class TypeImplementer
    {
        ...
        private readonly List<(string FieldName,LambdaExpression Converter)> _parameterConverters = new List<(string, LambdaExpression)>();
        ...
        private void ImplementType()
        {
            ...
            if (_interfaceType.IsSubclassOf(typeof(EventSource)))
                _typeBuilder = mb.DefineType(_interfaceType.FullName + "_Implemented",
                    TypeAttributes.Class | TypeAttributes.Public, _interfaceType);
            else if (_interfaceType.IsInterface)
                _typeBuilder = mb.DefineType(_interfaceType.FullName + "_Implemented",
                    TypeAttributes.Class | TypeAttributes.Public, typeof(EventSource), new Type[] {_interfaceType});
            else
                _typeBuilder = mb.DefineType(_interfaceType.FullName + "_Implemented",
                    TypeAttributes.Class | TypeAttributes.Public, typeof(EventSource));
            ...
            var baseCtor = typeof(EventSource).GetConstructor(BindingFlags.NonPublic | BindingFlags.Instance, null,
                new[] {typeof(bool)}, null);

            var emitter = _typeBuilder.DefineConstructor(MethodAttributes.Public,
                CallingConventions.Standard | CallingConventions.HasThis, new Type[0]).GetILGenerator();
            ...
            bool hasKeywords = (implementationAttribute.Keywords != null) ||
                               (FindNestedType(_interfaceType, "Keywords") != null);
            ...
            foreach (MethodInfo interfaceMethod in interfaceMethods)
            {
                ...
                if (implementationAttribute.ImplementComplementMethods &&
                    !_interfaceType.IsSubclassOf(typeof(EventSource)))
                {
                    var faultedMethod = EmitMethodFaultedImpl(invocationContext, beginMethod, ref eventId,
                        (EventKeywords) keywordForMethod);
                    EmitMethodCompletedImpl(invocationContext, beginMethod, ref eventId,
                        (EventKeywords) keywordForMethod, faultedMethod);
                }
            }
            ...
            t.GetField(_invocationContextsField.Name, BindingFlags.Static | BindingFlags.NonPublic)
                .SetValue(null, _invocationContexts.ToArray());
            t.GetField(_serializationProviderField.Name, BindingFlags.Static | BindingFlags.NonPublic)
                .SetValue(null, _serializationProvider);
            t.GetField(_contextProviderField.Name, BindingFlags.Static | BindingFlags.NonPublic)
                .SetValue(null, _contextProvider);

            foreach (var (fieldName, converter) in _parameterConverters)
            {
                _typeBuilder.GetField(fieldName, BindingFlags.Static | BindingFlags.Public)
