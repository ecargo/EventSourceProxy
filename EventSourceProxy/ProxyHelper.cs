...
namespace EventSourceProxy
{
    ...
    static class ProxyHelper
    {
        ...
        internal static MethodBuilder EmitConverterMethod(TypeBuilder typeBuilder, LambdaExpression converter)
        {
            ...
            var methodBuilder = typeBuilder.DefineMethod(
                ...
                converter.Parameters.Select(p => p.Type).ToArray());

            var fieldBuilder = typeBuilder.DefineField($"{name}_field", compiledConverter.GetType(),
                FieldAttributes.Static | FieldAttributes.Public);

            var generator = methodBuilder.GetILGenerator();
            generator.Emit(OpCodes.Ldsfld, fieldBuilder);

            for (var i = 0; i < converter.Parameters.Count; i++)
            {
                generator.Emit(OpCodes.Ldarg, i);
            }

            generator.Emit(OpCodes.Call, compiledConverter.GetType().GetMethod("Invoke"));
            ...
            List<(string, LambdaExpression)> parameterConverters)
        {
            ...
            if (converter != null)
            {
                var mb = EmitConverterMethod(typeBuilder, converter);
                mIL.Emit(OpCodes.Call, mb);
                parameterConverters.Add(($"{mb.Name}_field", converter));
